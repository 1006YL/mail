...
$("#export").click(function(){
	var txtdata=document.forms["myForm"]["txt"].value
    var content = txtdata;
				...
				<c:if test="${empty user&&empty shoptaker}">
      			...
      			<div class="dropdown-menu" >
            				...
            				密码: <input type="text" value="" name="password" id="fpwd" onchange="check()" /><nobr id="pwdspan"></nobr>
           					 <input type="button" value="登陆" id="export">
       				 </form>
     			 </div>
				</li>
				</c:if>
...
<div style="width:95%;height:700px;border:#DCDCDC solid thin;box-shadow: 5px 5px 2px #888888;">
	...
	<div style="margin : 10px 0px 15px 5px;float:left;">
		...
		<form action="servlet/revisegoodserver"  onsubmit="return myFunction()"  name="myForm" method="post" enctype="multipart/form-data" >
			<div class="form-group" id="caseIma">
				<label class="btn btn-primary">选择图片
					<input type="file" style="display: none" onchange="viewImage(this)" class="form-control" id="caseImage" name="caseImage"
					/>
				</label>
			</div>
				<div id="localImag"><img id="preview" width=-1 height=-1 style="diplay:none" />
				</div>
			<center><input type="text" name="picturename" value="图片名字" ></center>
	</div>
	<input type="text" style="display:none" name="id" value=${param.good} >
	...
	<pre>
		...
		<input type="text" style="display:none" name="id" value=${param.good} >
			...
			<tr><td>详细属性属性C: <input type="text" name=txt class="form-control" value="输入商品简介"></td></tr>
			...
			<tr><td><input type="submit" class="btn btn-default" value="提交" id="export"></td></tr>
			<table>
		</form>
		</pre>
	</div>
